...
brown = function(data, group){
  if ( length(unique(group)) != 2) stop('group not valide')
  ...
  p.one = min(p, 1-p)
  H1 = ifelse(median(x) > median(y),
              'Oneside test: M(1) > M(2)', 'Oneside test: M(1) < M(2)')
  list(H1.one=H1, p.one=p.one, 
       H1.two='Twoside test: M(1) != M(2)', p.two=2*p.one)
}
```
...
wilcoxon.two = function(data, group) {
  ...
  H1 = ifelse(median(data[cate]) > median(data[!cate]),
              'Oneside test: M(1) > M(2)', 'Oneside test: M(1) < M(2)')
...
pair.test = function(x, y){
  ...
  H1 = ifelse(w1 > w2,
              'Oneside test: M(x) > M(y)', 'Oneside test: M(x) < M(y)')
  list(H1.one=H1, p.one=p.one, 
       H1.two='Twoside test: M(x) != M(y)', p.two=2*p.one)}
```
